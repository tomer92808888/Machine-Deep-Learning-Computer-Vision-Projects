...
Original file is located at
    https://colab.research.google.com/drive/15K6NZLNYgZY697gnuv-9sVzJk90wY_pD
...
different scales of information. The first step is to combine these multi-scale features using an
element-wise sum, on which I apply Global Average Pooling (GAP) across the spatial
dimension. Next, I apply a channel- downscaling convolution layer to generate a compact
...
While the SKFF block fuses information across multi-resolution branches, the model also need a
...
order to maintain the residual nature of the architecture, residual resizing modules are
...
- I'm training MIRNet using **Charbonnier Loss** as the loss function and **Adam
Optimizer** with a learning rate of `1e-4`.
- I'm using **Peak Signal Noise Ratio** or PSNR as a metric which is an expression for the
