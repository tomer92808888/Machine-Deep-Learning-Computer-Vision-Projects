...
Original file is located at
    https://colab.research.google.com/drive/1BV7tYTHn5bstNrQVOl3cRYgv3-ufS7OL
...
"""## [Flickr8K](https://github.com/jbrownlee/Datasets/releases/download/Flickr8k)

This dataset comprises over
...
def load_captions_data(filename):
    ...
    with open(filename) as caption_file:
        ...
        for line in caption_data:
            ...
            # Remove caption that are either too short to too long
...
I used the `TextVectorization` layer to vectorize the text data,
that is to say, to turn the
original strings into integer sequences where each integer represents the index of
a word in a vocabulary. I also used a custom string standardization scheme
...
class ImageCaptioningModel(keras.Model):
    ...
    def train_step(self, batch_data):
        ...
        # 2. Pass each of the five captions one by one to the decoder along with the encoder outputs and compute the loss as well as accuracy for each caption.
    ...
    @property
    def metrics(self):
        # I need to list the metrics here so the `reset_states()` can be called automatically.
